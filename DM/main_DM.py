...
# config_path should be relative to the location of this file instead of the working directory
...
def main(args):
    ...
    if not hasattr(args, "eval_it") or args.eval_it == 0:
        ...
        eval_it_pool = np.arange(args.eval_it, args.Iteration+1, args.eval_it).tolist() if args.eval_mode == 'S' or args.eval_mode == 'SS' else [args.Iteration] # The list of iterations when we evaluate models and record results.
    ...
    for exp in range(args.num_exp):
        ...
        # images_all, labels_all = images_all.to(device), labels_all.to(device)
        ...
        # gs_model = nn.DataParallel(gs_model) if torch.cuda.device_count() > 1 else gs_model
        ...
        for it in range(args.Iteration+1):
            ...
            net = get_network(args.model, channel, num_classes, im_size, dist=False).to(device) # get a random model
            net.train()
